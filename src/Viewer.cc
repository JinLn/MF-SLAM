...
namespace ORB_SLAM3 {
    ...
    void Viewer::Run() {
        ...
        pangolin::CreateWindowAndBind("MF-SLAM: Map Viewer", 1024, 768);
        ...
        Twwp.SetIdentity();
        cv::namedWindow("MF-SLAM: Current Frame");
        ...
        while(1) {
            ...
            im.copyTo(tempim);
            if(tempim.cols > 700) {
                resize(tempim, tempim, cv::Size(tempim.cols / 2, tempim.rows / 2));
            }
            cv::imshow("MF-SLAM: Current Frame", toShow);
//        cv::imshow("MF-SLAM: Current Frame",tempim);
            cv::waitKey(mT);
